 ...
 * 28/01/13
 * - suppression des alias de fonction pour un code plus lisibe
 * - Suppression des constantes
 * - Changement des libellés des tips pour être certains que les modifications sont ok
 * ====================================================================
 *
 *
 */
...
//var    HAS_MARK = ':has(.mark)', // 2 occurrences

//var    DOT = ".", // :) used to turn class names into selectors
...
//var    STORAGE_KEY = "board";
var version = "0.6" ;
	
var    TRUE  = !0 + ""; // == "true"  //  little inconsistency, but I use "true" only as a string
var    FALSE = !1;      // == false   //  and false more as boolean
...
$(function () { // $(document).ready() -- theoretically not needed, as we don't manipulate the document contents
    ...
    data = data ? JSON.parse(data) : [
        { type: COLORS[0], text: "<p><i>Welcome to </i></p><b>Semantic Taskboard !</b> ("+version+")", top: 40, left: 70 },
...
// And finally the most interesting part - here it what is going on on the board
    ...
    $board
        ...
        .delegate(".card", "mousedown", function (mouseDownEvent) {
            ...
            if (!$body.hasClass("edit") && !$(mouseDownEvent.target).is(".tag" + "," + "menu a")) { // .hasClass
        ...
        .delegate(".tag", "click", function ($cards) { // var as param
            ...
            if ($(this).hasClass("mark")) { // .hasClass
                ...
                $body.not(':has(.mark)').removeClass("mark"); // .removeClass
            }
        });
...
// Last but not least, the event binded to the document itself.
    ...
    $document
        ...
        .bind("click", function ($target) {
            ...
            if ($body.hasClass("edit") && !$target.closest(".card" + "." + "edit")[0]) { // .hasClass
