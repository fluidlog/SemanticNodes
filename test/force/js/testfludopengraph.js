...
//	            	sparql_add_link(mousedown_node.iri_id, mouseup_node.iri_id);
	...
	function OpenFlud(d)
	{
		...
		p_el.select("#fo_closed_flud").remove();
		p_el.select("#fo_edit_option_image").remove();
		p_el.select("#fo_select_option_image").remove();
		p_el.select("#fo_edit_option_zone").remove();
		p_el.select("#fo_select_option_zone").remove();
	...
	function DisplayContentOpenedFlud(d)
	{
		...
		var textarea_label = field_label
					.append("textarea")
		...
		/* Close circle */
		var fo_close_option_image = p_el
					.append("circle")
					.attr("cx", -6+height_opened_flud/2)
					.attr("cy", 5-width_opened_flud/2)
					.attr("r", 13)
		...
		var fo_close_option_image = p_el
		    		.append("foreignObject")
		    		.attr("id", "fo_close_option_image")
					.attr('x', position_x_close_option_image_opened_flud)
					.attr('y', position_y_close_option_image_opened_flud)
					.attr('width', 22)
					.attr('height', 22)

		//xhtml div image
		var fo_xhtml_close_image = fo_close_option_image
					.append('xhtml:div')
				    .attr("class", "fo_div_image")
				    .attr("id", "fo_div_close_option_image")
					.append('i')
				    .attr("id", "fo_i_close_option_image")
				    .attr("class", "ui large close icon")
					.attr("style", "display:inline")

		/* Close zone */
		var fo_close_option_zone = p_el
					.append("foreignObject")
		    		.attr("id", "fo_close_option_zone")
					.attr('x', -7+width_opened_flud/2-width_closed_flud/2)
					.attr('y', 7-height_opened_flud/2-height_closed_flud/2)
					.attr('width', width_closed_flud)
					.attr('height', height_closed_flud)

		//xhtml div zone (with 40px of padding for Chrome to create a hovered surface
		var fo_xhtml_close_option_zone = fo_close_option_zone
					.append('xhtml:div')
					.attr("style", "padding:40px; text-align:right;"
							+"background-color:rgba(200,200,200,.1);")

		//action on close zone
		var action_on_close_option_zone = fo_close_option_zone
					.style("cursor", "pointer")
					.on("click", CloseFlud)
					.append("title").text("Click to close flud")
	...
	function CloseFlud()
	{
        ...
        p_el.select("#circle_close").remove();
        p_el.select("#fo_i_drag_option_image").remove();
        p_el.select("#fo_i_type_option_image").remove();
        p_el.select("#fo_close_option_image").remove();
        p_el.select("#fo_close_option_zone").remove();
		...
		p_el
			...
			.attr("width", width_closed_flud)
			.attr("height", height_closed_flud)
			.each("end", DisplayContentClosedFlud(p_el));
